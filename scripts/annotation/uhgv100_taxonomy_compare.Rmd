...
ct3_tax_dt<- fread(
  sprintf(
    "%s/data/annotation/uhgv/ct3/ct3_100uhgv_332_virus_summary.tsv",
    find_rstudio_root_file()
    )
  ) 
...
ct3_parse_dt <- ct3_tax_dt %>%
  ...
  mutate(
    realm = str_extract(taxonomy_hierarchy, "(?<=d_).*?(?=;|$)"),
    domain = str_extract(taxonomy_hierarchy, "(?<=-_).*?(?=;|$)"),
...
both_dt <- rbind(ct3_parse_dt,
                 gnmd_parse_dt) %>%
  group_by(input_name) %>%
  mutate(ct3_domain = domain[tool == "Cenote-Taker 3"]) %>%
  pivot_longer(!c(input_name, tool, ct3_domain),
               names_to = "tax_level",
               values_to = "taxon") %>%
...
tax_p <- both_dt %>%
  ...
  geom_tile() +
  facet_grid(rows = vars(ct3_domain), cols = vars(tool), 
             space = "free",
             scales = "free_y") +
  theme(
