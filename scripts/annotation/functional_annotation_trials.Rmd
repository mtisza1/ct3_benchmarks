...
```{r}
phar_dir <- sprintf(
  "%s/data/annotation/genbank/gene_to_contig/pharokka",
  find_rstudio_root_file())

ct3_dir <- sprintf(
  "%s/data/annotation/genbank/gene_to_contig/ct3",
  find_rstudio_root_file())

genomad_dir <- sprintf(
  "%s/data/annotation/genbank/gene_to_contig/genomad",
  find_rstudio_root_file())

metacerberus_dir <- sprintf(
  "%s/data/annotation/genbank/gene_to_contig/metacerberus",
  find_rstudio_root_file())
...
## list of relevant files in the directory
gnmd_files <- list.files(genomad_dir, pattern = "*.tsv", full.names = TRUE)



for (file in gnmd_files){
  ...
  if (!exists("gmnd_dataset")){
    ...
    gmnd_dataset <- fread(file, header=T, sep="\t") %>%
      mutate(contig = sub("_[^_]+$", "", gene)) %>%
      group_by(contig) %>%
      summarize(gene_count = n(),
                hypo_annotations = 
                  n_distinct(gene[
                    is.na(annotation_description)
                    ]
                    )) %>%
      mutate(tool = "genomad",
             settings = "annotate",
                 tax_label = str_split_i(tax_label, "_", 1)[1])

  }
  
  # if the merged dataset does exist, append to it
  else if (exists("gmnd_dataset")){
    ...
    gmnd_dataset<-rbind(gmnd_dataset, temp_dataset)
    rm(temp_dataset)
  }
}
...
## list of relevant files in the directory
metc_files <- list.files(metacerberus_dir, pattern = "*.tsv", full.names = TRUE)



for (file in metc_files){
  ...
  if (!exists("metc_dataset")){
    ...
    metc_dataset <- fread(file, header=T, sep="\t") %>%
      mutate(contig = sub("_[^_]+$", "", locus_tag)) %>%
      group_by(contig) %>%
      summarize(gene_count = n(),
                hypo_annotations = 
                  n_distinct(locus_tag[
                    product %in% c("hypothetical protein",
                                   "hypothetical",
                                   "Hypothetical")
                    ]
                    )) %>%
      mutate(tool = "metacereberus",
             settings = "all DBs",
                 tax_label = str_split_i(tax_label, "_", 1)[1])
  ...
  # if the merged dataset does exist, append to it
    ...
    metc_dataset<-rbind(metc_dataset, temp_dataset)
    rm(temp_dataset)
  }
}
...
comb_dt <- rbindlist(list(ct3_dataset, phar_dataset, 
                       gmnd_dataset, metc_dataset))
...
annotp <- comb_dt %>%
  mutate(annotated_perc = (1 - (hypo_annotations/gene_count))) %>%
  filter(tax_label != "tubulavirales") %>%
  ggplot(aes(tool, annotated_perc)) +
  geom_boxplot(aes(fill = tool), outlier.shape = NA) + 
  ...
  stat_compare_means(label = "p.signif",
                     comparisons = 
                       rev(list(c("Cenote-Taker 3", "genomad"), 
                            c("Cenote-Taker 3", "metacereberus"),
                            c("Cenote-Taker 3", "pharokka"),
                            c("genomad", "metacereberus"),
                            c("genomad", "pharokka"),
                            c("metacereberus", "pharokka")
                            ))) +
...
ggsave(annotp,
       file = sprintf(
  "%s/charts/annotation_rates_by_tool.pdf",
  find_rstudio_root_file()),
  width = 8, height = 6)
```
...
```{r}
autolyk_metc_dt <- fread(sprintf(
  "%s/data/annotation/gene_to_contig/metacerberus/autolykiviridae_annotation_summary.tsv",
  find_rstudio_root_file()
))
...
```{r}
autolyk_ct3_dt <- fread(sprintf(
  "%s/data/annotation/gene_to_contig/ct3/autolykiviridae.genes.ct3.tsv",
  find_rstudio_root_file()
))

autolyk_ct3_dt %>%
  group_by(evidence_description) %>%
  mutate(evidence_description = str_trunc(evidence_description, 40)) %>%
  summarize(n = n())
