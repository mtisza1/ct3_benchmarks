---
title: "taxonomy classification seawater dtr, ct3 and genomad"
...
```{r}
wd <- "/Users/michaeltisza/Library/CloudStorage/OneDrive-BaylorCollegeofMedicine/virus_annotation_benchmarks/real_data/seawater_dtrs"
```
...
gnmd_tax_dt <- fread(sprintf(
  "%s/genomad/seawater_scaffolds_gt1500.dtrs_itr_only.20k_taxonomy.tsv",
                              wd)) %>%
  filter(n_genes_with_taxonomy >= 10 | grepl("Malgrand", lineage))

ct3_tax_dt<- fread(sprintf(
  "%s/ct3/seawater_dtrs_1a_virus_summary.tsv",
                              wd)) %>%
