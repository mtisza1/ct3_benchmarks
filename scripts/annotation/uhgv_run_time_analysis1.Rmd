...
time_metric1000_dt <- time_metricd_dt %>%
  filter(`run name` %in% c("gnmd_uhgv1000_time_a",
                           "ct3time_1000uh1", 
                           "pharokka_1000uh2")
         ) %>%
  mutate(tool = case_when(software == "CT3" ~ "Cenote-Taker 3",
...
ggsave(
  time_1000p,
  file = sprintf("%s/charts/time_1000uhgv_bar2.pdf",
          find_rstudio_root_file()),
  height = 5,
  width = 3
)
...
ggsave(
  time_1000p,
  file = sprintf("%s/charts/time_1000uhgv_bar2.png",
          find_rstudio_root_file()),
  height = 5,
  width = 3
)
